    ...
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border/40">
      <nav className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between h-14 lg:h-16">
        <Link href="/" className="flex items-center gap-1">
          <span className="text-xl lg:text-2xl font-serif italic text-foreground tracking-tight">microcemento</span>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-4">
          <a
            ...
            className="inline-flex items-center gap-2 bg-foreground text-background text-[13px] px-5 py-2 hover:opacity-80 transition-opacity"
          >
      ...
      {open && (
        <div className="lg:hidden border-t border-border bg-background px-6 py-6 space-y-4">
          ...
          <a href={WA_URL} target="_blank" rel="noopener noreferrer" className="block text-center bg-foreground text-background text-sm px-6 py-3">
            <MessageCircle className="w-4 h-4 inline mr-2" /> Contactar por WhatsApp
          </a>
        </div>
      )}
    </header>
